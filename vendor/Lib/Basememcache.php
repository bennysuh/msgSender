...
class Basememcache{
    ...
    public function __construct($memconf)
    {
        if(!isset(\Config\Db::$$memconf))
        {
            echo "\\Config\\Db::$memconf not set\n";
            throw new \Exception("\\Config\\Db::$memconf not set\n");
        }
        $config = \Config\Db::$$memconf;
        $this->MEMCACHE_HOST = $config['host'];
        $this->MEMCACHE_PORT = $config['port'];
        $this->MEMCACHE_EXPIRATION = $config['expire'];
        $this->MEMCACHE_PREFIX = $config['prefix'];
        $this->MEMCACHE_COMPRESSION = $config['compression'];
